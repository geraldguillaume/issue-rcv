/*  =========================================================================
    issue_rcv_selftest.c - run selftests
...
// Tests for draft public classes:
    { "issue_rcv_server", issue_rcv_server_test, false, true, NULL },
...
{
    test_item_t *item;
    printf ("Running issue-rcv selftests...\n");
    for (item = all_tests; item->testname; item++) {
        if (streq (item->testname, "private_classes"))
            continue;
        if (!item->subtest)
            item->test (verbose);
#ifdef ISSUE_RCV_BUILD_DRAFT_API // selftest is still in draft
        else
            issue_rcv_private_selftest (verbose, item->subtest);
#endif // ISSUE_RCV_BUILD_DRAFT_API
    }
...
main (int argc, char **argv)
{
    ...
    for (argn = 1; argn < argc; argn++) {
        ...
        ||  streq (argv [argn], "-h")) {
            puts ("issue_rcv_selftest.c [options] ...");
    ...
    if (test) {
        printf ("Running issue-rcv test '%s'...\n", test->testname);
        if (!test->subtest)
            test->test (verbose);
#ifdef ISSUE_RCV_BUILD_DRAFT_API // selftest is still in draft
        else
            issue_rcv_private_selftest (verbose, test->subtest);
#endif // ISSUE_RCV_BUILD_DRAFT_API
    }
    else
        test_runall (verbose);
